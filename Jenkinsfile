pipeline {
  ...
  stages {
    ...
    stage('sign') {
      steps {
        sh 'bash -c \'cd /code && . build/envsetup.sh && croot && /code/build/tools/releasetools/sign_target_files_apks -o -d /tmp/android-certs $OUT/obj/PACKAGING/target_files_intermediates/*-target_files-*.zip /tmp/signed-target_files.zip\''
