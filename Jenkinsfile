  ...
  stages {
    ...
    stage('sign') {
      steps {
        ...
        sh '''base -c \'cd /code && . build/envsetup.sh && lunch lineage_vangogh-user && $OUT/build/tools/releasetools/ota_from_target_files -k /tmp/android-certs/releasekey --block --backup=true /tmp/signed-target_files.zip 
