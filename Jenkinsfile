pipeline {
  ...
  stages {
    ...
    stage('sign') {
      steps {
        sh 'bash -c \'cd /code && . build/envsetup.sh && breakfast $BUILD_TARGET && mka target-files-package otatools && croot && python2 /code/out/host/linux-x86/bin/sign_target_files_apks -v -o -d /tmp/android-certs /code/out/target/product/*/obj/PACKAGING/target_files_intermediates/*-target_files-*.zip /tmp/signed-target_files.zip\''
